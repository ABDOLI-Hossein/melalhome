...
import { booleanConv } from "./utils/boolean";
...
function PostDetail({match, user, history}) {
    ...
    useEffect(() => {
        async function getData(){
            const res = await axios.get(`http://localhost:3001/${match.params.posttype}/postdetail/?id=${match.params.id}`).then(res => {
                setRents(res.data)
                setLoading(false)
                console.log(res)
            })
    ...
    const deletePost = () => {
        async function getData(){
            const res = await axios.delete(`http://localhost:3001/admin/delete/?id=${match.params.id}&post_type=${match.params.posttype}`).then(res => {
                setLoading(false)
    ...
    const publishPost = () => {
        async function getData(){
            const res = await axios.put(`http://localhost:3001/admin/publish/?id=${match.params.id}&post_type=${match.params.posttype}`).then(res => {
                setLoading(false)
                console.log(res)
                res.status === 200 ? Swal.fire({
                    text: ' آگهی منتشر شد',
            ...
            <div className="w-50">
                        ...
                        <div className="w-100 float-right my-10">
                            ...
                            <div className="float-right w-100">
                                <div className={"float-left ltr btnCopy"} >
                                    ...
                                    <p className={"float-left m-0 text-red"} onClick={()=>{copy('09124646464')}}>{e2p(rents.mobile)}</p>
                                    
                                </div>
                            </div>
                            <p className="descriptionInformation float-right w-100">
                                <h4>هشدار پلیس</h4>
                                <p>لطفا پیش از انجام معامله و هر نوع پرداخت وجه . از صحت کالا یا خدمات اراپه شده . به
                                    صورت حضوری اطمینان حاصل نمایید</p>
                            </p>
                        </div> : ''
                    }
                </div>
                ...
                <div className="mainInformation">
                    ...
                    <ul>
                        ...
                        <li><p>   کف‌پوش  : {booleanConv(rents.floorـcovering)}</p></li>
